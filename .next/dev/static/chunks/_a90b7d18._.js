...
function CreateCampaignModal({ open, onOpenChange, onViewCampaigns, onSubmit }) {
        ...
        reader.onload = (event)=>{
            try {
                ...
                setFormData({
                    ...
                    numberOfTargets: formData.emails.length + newEmails.filter((e)=>e.email).length
                });
            } catch (err) {
    ...
    const handleSupabaseImport = async ()=>{
        // Mock Supabase import - in real app, this would call Supabase API
        console.log("Importing from Supabase:", supabaseConfig);
        ...
        setFormData({
            ...
            numberOfTargets: formData.emails.length + mockEmails.length
        });
    };
...
_s(CreateCampaignModal, "dwtuR1Ct9Z4LKxTmvyuNmtD37IY=");
    ...
    const generateEvents = ()=>{
        ...
        if (email.status === "delivered" || email.status === "bounced" || email.status === "clicked") {
            events.push({
                id: "2",
                type: "delivered",
                timestamp: baseDate.replace(/\d+:\d+:\d+/, (match)=>{
                    const [h, m, s] = match.split(":");
                    const newS = String(parseInt(s) + 1).padStart(2, "0");
                    return `${h}:${m}:${newS}`;
                }),
        ...
        if (email.status === "clicked") {
            events.push({
                ...
                timestamp: baseDate.replace(/\d+:\d+:\d+/, (match)=>{
                    ...
                    const newM = String(parseInt(m) + 5).padStart(2, "0");
                    return `${h}:${newM}:00`;
                }),
        ...
        if (email.status === "bounced") {
            events.push({
                ...
                timestamp: baseDate.replace(/\d+:\d+:\d+/, (match)=>{
                    const [h, m, s] = match.split(":");
                    const newS = String(parseInt(s) + 2).padStart(2, "0");
                    return `${h}:${m}:${newS}`;
                }),
...
_s(DashboardView, "r0DpVFmU07IBaq7YI0R5AjqxTUc=", false, function() {
    return [
...
//# sourceMappingURL=_a90b7d18._.js.map